...
#well matched? 
summary(data$Diagnosis) #numerically, yes. (ASD: 166, TD: 186)
...
summary(mixed_model) #we need to interpret 
...
#evaluating them 
library(lmerTest)
summary(m_cubic) #needs interpretation
summary(m_quadratic) #needs interpretation 
summary(m_linear)

#comparing them 
anova(m_linear, m_quadratic, m_cubic) #right now, more complex is better - do we have penalty?
...
library(lmerTest)
corrplot(corr,method="number",col=brewer.pal(n=8, name="PuOr"),type="upper",tl.col="darkblue")
corrplot(corr,col = brewer.pal(n=8, name="PuOr"), type = "upper", tl.col="darkblue") #different way

#stepwise 
basis <- lm(CHI_MLU ~ VISIT + Gender + Ethnicity + ADOS_1 + nonVerbalIQ_1 + verbalIQ_1 + MOT_MLU, data = model_data)

#stepwise w. lmer 
big_model <- lmer(CHI_MLU ~ VISIT + Gender + Ethnicity + ADOS_1 + nonVerbalIQ_1 + verbalIQ_1 + MOT_MLU + (1+VISIT|SUBJ), data = model_data)

summary(big_model) #some NAs

step1 <- step(big_model)
summary(step1) 
