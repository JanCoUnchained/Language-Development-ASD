...
```{r}
demo <- demo_raw %>%
  rename(SUBJ = Child.ID, VISIT = Visit) #Here we rename the Child.ID to SUBJ, and capitalize visit 
```
...
#Here we changes Visit to a character as str_extract doesn't work on factor
token <- token_raw %>%
  mutate(VISIT = as.character(VISIT),
         VISIT = str_extract(VISIT, "(\\d)+"),
         VISIT = as.integer(VISIT))
...
#Here we rename the children in order to standardize it. The problem is that this dataset the children without abbreviations has a dot at the end of their name. This function to finds any lowercase letters, as they are only present in the relevant cases. Then it removes the last character. In order to prevent it to rename them to numbers, we ask it to be as characters. 
```
...
#First we have a list of needed variables

demo_3 <- demo %>%
  select(one_of(int_var)) %>%
  rename(nonVerbalIQ = MullenRaw, verbalIQ = ExpressiveLangRaw)
...
#Here it goes through the list and takes what is needed. For the demo part it renames two of the columns to make it more understandable
```
...
```{r}
data <- demo_3 %>%
  left_join(lu_3, by = c("SUBJ", "VISIT")) %>%
  left_join(token_3, by = c("SUBJ", "VISIT"))

#Left join keeps our first dataframe from before, in this case demo_3, and then append the other columns onto it
  

rm(list=setdiff(ls(), "data")) 
# https://stackoverflow.com/questions/6190051/how-can-i-remove-all-objects-but-one-from-the-workspace-in-r
#Used to clean up the enviroment
```
...
#First we select certain columns, while renaming a few due to merging later
#Then we filter away the visits that are not the first
#Then we only want certain variables so we remove the rest
...
data_3$Gender <- if_else(data_3$Gender=="1", "F", "M")

data_3$Diagnosis <- if_else(data_3$Gender=="A", "ASD", "TD")
```
