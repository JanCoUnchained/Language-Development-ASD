...
token <- token_raw %>%
  mutate(VISIT = str_extract(VISIT, "[0-9]"), #same as above
...
demo_2 <- demo %>%
  mutate(SUBJ = str_remove_all(SUBJ, "[:punct:]"))
...
data <- demo_3 %>% #demo has to be first because it is the biggest
  full_join(lu_3, by = c("SUBJ", "VISIT")) %>% #Joining SUBJ & VISIT which are common to all 
  full_join(token_3, by = c("SUBJ", "VISIT")) #Left join ensures that it the incomplete rows are kept. 


rm(list=setdiff(ls(), c("data", "demo_3", "lu_3", "token_3"))) 
# removing everything but the "data" from workspace.
...
# missing lingo
data_4 <- data_3 %>%
  filter(!(is.na(MOT_MLU) & is.na(CHI_MLU))) %>%
  mutate(Ethnicity = case_when(Ethnicity == "Bangledeshi" ~ "Bangladeshi",
                               TRUE ~ as.character(Ethnicity)))

table(data_4$Ethnicity)
...
```{r}
write.csv(data_4, "language_dev_asd_clean_2.csv", row.names = F)
```
