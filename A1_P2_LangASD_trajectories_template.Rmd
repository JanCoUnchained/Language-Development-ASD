...
library(pacman)
p_load(tidyverse, lme4, data.table)
```
...
#setwd(locpath)
#Data =
data = read.csv("language_dev_asd_clean.csv", sep = ",")
...
dataTD <- data %>% filter(Diagnosis == "TD") %>% filter(VISIT == "1")
dataASD <- data %>% filter(Diagnosis == "ASD") %>% filter(VISIT == "1")

table(data$Gender)
table(dataTD$Gender)
table(dataASD$Gender) 

data %>% group_by(data$Diagnosis) %>% table(data$Gender)


data <- data[rowSums(is.na(data[ , 8:17])) < 10, ]


data[rowSums(is.na(data1[ , 8:17])) > 10, ]

data %>% group_by(Diagnosis) %>% summarise(ADOS_1) 
summarise(data$TD
```
