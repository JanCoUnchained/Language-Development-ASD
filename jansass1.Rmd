...
```{r}
demo_raw <- read.csv("data/demo_train.csv")
lu_raw <- read.csv("data/LU_train.csv")
token_raw <- read.csv("data/token_train.csv")
```
...
```{r}
demo <- demo_raw %>%
  rename(SUBJ = Child.ID, VISIT = Visit)
```
...
```{r}
write.csv(data_3, "ass1_clean_data.csv", row.names = F)
```
